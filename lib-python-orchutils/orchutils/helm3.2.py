...
def install_chart(chart, version, values, release, namespace, validate_manifest=False):
    """
...
def upgrade_chart(chart, version, values, release, namespace):
    """
    ...
    values_file = create_values_file(values)
    try:
        deploy_cmd = '{helm} upgrade --values {values_file} {release} {chart} --version {version} --namespace {namespace} --dry-run'.format(
            helm=helm_binary,
