...
def install_chart(chart, version, values, release, namespace, validate_manifest=False, dry_run=False, debug=False):
    ...
    try:
        (rc, output) = baseutils.exe_cmd('{helm} install  {release} {chart} --version {version} --namespace {namespace} --dry-run {debug}'.format(
            helm=helm_binary,
        ...
        deploy_cmd = '{helm} install {release} {chart} --version {version} --namespace {namespace} {dry_run} {debug}'.format(
            helm=helm_binary,
            release=baseutils.shell_escape(release),
            chart=baseutils.shell_escape(chart),
            version=baseutils.shell_escape(version),
